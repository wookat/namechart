(function () {
  ...
  if (list) {
    ...
    var renderActions = function () {
      var acts = document.getElementById('nc-fav-actions');
      if (!acts) return;
      var f = favs();
      var chips = [];
      for (var i = 0; i < f.length - 1 && chips.length < 4; i++) {
        var s1 = String(f[i].slug).replace(/[^a-z'-]/g, ''), s2 = String(f[i + 1].slug).replace(/[^a-z'-]/g, '');
        var pair = s1 < s2 ? s1 + '-vs-' + s2 : s2 + '-vs-' + s1;
        chips.push('<a href="/compare/' + pair + '" class="px-3 py-1.5 rounded-full bg-white border border-slate-200 hover:border-indigo-400">Compare ' + String(f[i].name).replace(/[<>&"]/g, '') + ' vs ' + String(f[i + 1].name).replace(/[<>&"]/g, '') + '</a>');
      }
      var mq = f.slice(0, 3).map(function (x) { return 'names=' + encodeURIComponent(String(x.slug).replace(/[^a-z\'-]/g, '')); }).join('&');
      acts.innerHTML = f.length >= 2
        ? '<p class="text-sm font-semibold text-slate-700 mb-2">Do more with your shortlist:</p><div class="flex flex-wrap gap-2 text-sm">' + chips.join('') + '<a href="/matcher?' + mq + '" class="px-3 py-1.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100 hover:border-indigo-400">Find sibling &amp; middle names \u2192</a></div>'
        : f.length === 1
          ? '<div class="flex flex-wrap gap-2 text-sm"><a href="/matcher?' + mq + '" class="px-3 py-1.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100 hover:border-indigo-400">Find sibling &amp; middle names for ' + String(f[0].name).replace(/[<>&"]/g, '') + ' \u2192</a></div>'
          : '';
    };
    renderActions();
