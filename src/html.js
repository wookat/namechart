...
<header class="bg-white border-b border-slate-200 sticky top-0 z-20">
  <div class="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between gap-3">
    ...
    <nav class="flex items-center gap-3 text-sm font-medium text-slate-600">
      ...
      <a class="hover:text-indigo-700 hidden sm:inline" href="/trending">Trending</a>
      <a class="hover:text-indigo-700 hidden md:inline" href="/browse">Browse</a>
    </nav>
  </div>
...
<footer class="bg-white border-t border-slate-200 mt-10">
  ...
  <div class="border-t border-slate-100 py-4 text-center text-xs text-slate-400">© ${new Date().getFullYear()} NameChart · A Zalize project</div>
</footer>
<script>
...
export function chartSVG(series, { width = 800, height = 260 } = {}) {
...
export function emailForm() {
  ...
  <p class="mt-1 text-indigo-100 text-sm">Occasional updates when we add new data & tools. No spam, unsubscribe anytime.</p>
...
export function nameCard(r) {
  const sexBadge = r.f_total && r.m_total
